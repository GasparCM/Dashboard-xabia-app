...
import { Save, Upload, Plus, Trash2, Eye, EyeOff, Globe, Palette, Bot } from 'lucide-react';
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-text-title-strong text-lg font-medium">Configuración del Sistema</h2>
          <p className="text-text-body mt-1">Gestiona la configuración global de Javea2Live</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          {activeTab === 'branding' && (
            <Card>
              <h3 className="text-text-title-strong font-medium mb-6">Configuración de Marca</h3>
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-text-title mb-2">Logo</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-text-title mb-2">Título de la Aplicación</label>
                  <input
                    type="text"
                    value={settings.branding.title}
                    onChange={(e) => setSettings({
                      ...settings,
                      branding: { ...settings.branding, title: e.target.value }
                    })}
          ...
          {activeTab === 'home' && (
            <Card>
              <h3 className="text-text-title-strong font-medium mb-6">Bloques de la Página Principal</h3>
                  ...
                  <div key={block.id} className="flex items-center justify-between p-4 border border-border rounded-lg">
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="font-medium text-text-title-strong">{block.title}</h4>
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => {
                          ...
                          newBlocks[index].isActive = !newBlocks[index].isActive;
          ...
          {activeTab === 'translations' && (
            <Card>
              <h3 className="text-text-title-strong font-medium mb-6">Gestión de Traducciones</h3>
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {Object.entries(settings.translations.es).map(([key, value]) => (
                    <div key={key} className="grid grid-cols-4 gap-4 items-center">
                      <div className="font-mono text-sm text-text-body">{key}</div>
                      <input
                        type="text"
          ...
          {activeTab === 'ai' && (
            <Card>
              <h3 className="text-text-title-strong font-medium mb-6">Configuración de IA</h3>
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-text-title mb-2">Proveedor de IA</label>
                  <select
                  ...
                  <p className="text-xs text-text-body mt-1">
                    Selecciona el proveedor de IA para funciones automáticas (solo UI)
                  </p>
                </div>
                <div className="bg-warning/10 border border-warning/20 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-warning mb-1">Configuración Solo UI</h4>
                      <p className="text-sm text-text-body">
                        Esta configuración es solo para demostración. En un entorno real, 
                        aquí se configurarían las claves API y parámetros del proveedor de IA.
                      </p>
                    </div>
                  </div>
                </div>
